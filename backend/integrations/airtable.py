...
authorization_url = f'https://airtable.com/oauth2/v1/authorize?client_id={CLIENT_ID}&response_type=code&owner=user&redirect_uri={encoded_redirect}'

encoded_client_id_secret = base64.b64encode(f'{CLIENT_ID}:{CLIENT_SECRET}'.encode()).decode()
# Use a more limited scope to start with - you can expand this after confirming it works
...
async def authorize_airtable(user_id, org_id):
    ...
    encoded_scope = requests.utils.quote(scope, safe='')
    auth_url = f'{authorization_url}&state={encoded_state}&code_challenge={code_challenge}&code_challenge_method=S256&scope={encoded_scope}'
